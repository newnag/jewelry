function add(){
    ...
    data.append('tax_id',document.querySelector('#tax_id').value)
    ...
    $.ajax({
      ...
      success:function(result){ 
        Swal.fire({
            ...
            location.href = "https://thavorn-jewelry.com/stock-jewelry/supplier/"
        })
      },
    ...
    $.ajax({
      ...
      success:function(result){
        ...
        if(result['type_supplier'] == 1){
          txt = "ทอง"
          option = `<option value="${result['type_supplier']}">${txt}</option><option value="2">กรอบพระ</option>`
        }
        else if(result['type_supplier'] == 2){
          txt = "กรอบพระ"
          option = `<option value="${result['type_supplier']}">${txt}</option><option value="1">ทอง</option>`
        }
        else if(result['type_supplier'] == 3){
          txt = "เพชร"
          option = `<option value="${result['type_supplier']}">${txt}</option><option value="3">เพชร</option>`
        }
...
function update(){
    ...
    data.append('tax_id',document.querySelector('#tax_id_edit').value)
