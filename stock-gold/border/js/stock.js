    ...
    $.ajax({
      ...
      success:function(result){ 
        Swal.fire({
            ...
            location.href = "https://thavorn-jewelry.com/stock-gold/border"
        })
      },
...
function barcode_page(){
  window.open("https://thavorn-jewelry.com/stock-gold/border/get-barcode.php",'_blank')
}
      ...
      for (i=0;i<result.length;i++){
        template = `
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="product_no" id="chk${i}" value="${result[i].product_no}" data-type="${result[i].type_name}" data-weight="${result[i].weight}" data-size="${result[i].size}" data-wage="${result[i].wage}">
